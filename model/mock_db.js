class userStorage {
  ...
  searchUser(username) {
    let users = Object.values(this.#storage).filter((user) => {
      return user.firstname === username || user.lastname === username;
    });
  ...
  createUser(firstname, lastname) {
    try {
      this.#id += 1;
      this.#storage[this.#id] = {id:this.#id,firstname: firstname, lastname: lastname };
